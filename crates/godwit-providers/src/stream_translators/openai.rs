...
impl OpenAiStreamTranslator {
    ...
    pub fn translate_chunk(
        ...
        let parsed: OpenAiStreamChunk = match serde_json::from_str(chunk) {
            Ok(v) => v,
            Err(e) => {
                return vec![Ok(SseEvent {
                    data: build_sse_error(&format!("failed to parse openai chunk: {e}")),
                })];
            }
        };
...
impl Default for OpenAiStreamTranslator {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_malformed_chunk_produces_error() {
        ...
        assert!(data["message"].as_str().unwrap().contains("failed to parse openai chunk"));
    }
