...
impl VllmStreamTranslator {
    ...
    pub fn translate_chunk(
        ...
        let parsed: VllmStreamChunk = match serde_json::from_str(chunk) {
            Ok(v) => v,
            Err(e) => {
                return vec![Ok(SseEvent {
                    data: build_sse_error(&format!("failed to parse vllm chunk: {e}")),
                })];
            }
        };
...
impl Default for VllmStreamTranslator {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_translate_delta_chunk() {
        let mut translator = VllmStreamTranslator::new();
    ...
    fn test_malformed_chunk_produces_error() {
        ...
        assert!(data["message"].as_str().unwrap().contains("failed to parse vllm chunk"));
    }
