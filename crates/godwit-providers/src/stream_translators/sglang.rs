...
impl SglangStreamTranslator {
    ...
    pub fn translate_chunk(
        ...
        let parsed: SglangStreamChunk = match serde_json::from_str(chunk) {
            Ok(v) => v,
            Err(e) => {
                return vec![Ok(SseEvent {
                    data: build_sse_error(&format!("failed to parse sglang chunk: {e}")),
                })];
            }
        };
...
impl Default for SglangStreamTranslator {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_translate_delta_chunk() {
        let mut translator = SglangStreamTranslator::new();
    ...
    fn test_malformed_chunk_produces_error() {
        ...
        assert!(data["message"].as_str().unwrap().contains("failed to parse sglang chunk"));
    }
    ...
    fn test_finish_only_emitted_once() {
        ...
        let chunk1 = r#"{"choices":[{"index":0,"delta":{"content":""},"finish_reason":"stop"}],"usage":{"prompt_tokens":10,"completion_tokens":5,"total_tokens":15}}"#;
        let chunk2 = r#"{"choices":[{"index":0,"delta":{"content":""},"finish_reason":"stop"}],"usage":{"prompt_tokens":20,"completion_tokens":10,"total_tokens":30}}"#;
    ...
    fn test_empty_content_ignored() {
        let mut translator = SglangStreamTranslator::new();
        let chunk = r#"{"choices":[{"index":0,"delta":{"content":""},"finish_reason":null}]}"#;
    ...
    #[test]
    fn test_chunk_without_finish_reason() {
        let mut translator = SglangStreamTranslator::new();
        let chunk = r#"{"choices":[{"index":0,"delta":{"content":"Hello"},"finish_reason":null}]}"#;
