...
impl LlamaCppStreamTranslator {
    ...
    pub fn translate_chunk(
        ...
        let parsed: LlamaCppStreamChunk = match serde_json::from_str(chunk) {
            Ok(v) => v,
            Err(e) => {
                return vec![Ok(SseEvent {
                    data: build_sse_error(&format!("failed to parse llama.cpp chunk: {e}")),
                })];
            }
        };
        ...
        if parsed.stop == Some(true) && !self.finish_emitted {
...
impl Default for LlamaCppStreamTranslator {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_translate_content_chunk() {
        let mut translator = LlamaCppStreamTranslator::new();
    ...
    fn test_translate_finish_chunk_with_usage() {
        let mut translator = LlamaCppStreamTranslator::new();
        let chunk = r#"{"stop":true,"usage":{"prompt_tokens":10,"completion_tokens":5}}"#;
    ...
    fn test_multiple_content_then_finish() {
        ...
        let chunk1 = r#"{"content":"Hello"}"#;
        let chunk2 = r#"{"content":" world"}"#;
        let chunk3 = r#"{"stop":true,"usage":{"prompt_tokens":10,"completion_tokens":5}}"#;
    ...
    fn test_malformed_chunk_produces_error() {
        ...
        assert!(data["message"].as_str().unwrap().contains("failed to parse llama.cpp chunk"));
    }
    ...
    fn test_finish_only_emitted_once() {
        ...
        let chunk1 = r#"{"stop":true,"usage":{"prompt_tokens":10,"completion_tokens":5}}"#;
        let chunk2 = r#"{"stop":true,"usage":{"prompt_tokens":20,"completion_tokens":10}}"#;
    ...
    #[test]
    fn test_chunk_without_stop_field() {
        let mut translator = LlamaCppStreamTranslator::new();
        let chunk = r#"{"content":"Hello","stop":false}"#;
